...
<img src="https://img.shields.io/badge/pdfplumber-555555?style=flat-square&logo=python&logoColor=white" alt="pdfplumber" height="30">
...
# 소감
🐸이태호(팀장) : \
pdf데이터를 OCR하여 벡터DB에 임베딩하는 과정에서 단순히 넣기만 하면 되는것이 아니라 표 데이터라던지 수치적인 데이터를 따로 처리해줘야 한다는것을 깨달았습니다. 저희팀의 경우 RDB에서 수치데이터를 같이 불러와서 답변하도록 구현하였는데, 이 과정에서 rag시스템에 대한 전반적인 이해도가 늘었던 것 같아 좋았습니다.  

🐯김영우 : \
기술 구현이라는거 정말 쉽지 않은 분야인거 같고 공부가 많이 필요해 보여서 시간 투자를 많이 해야겠다라고 느꼈습니다!  

🐹임승옥 : \
데이터 전처리부터, LangGraph 설계, multi-agent구축, 프롬프트 engineering까지 전반적인 rag 시스템을 구현을 함으로써, 전반적인 rag시스템에 대해 이해할 수 있어서 좋았습니다. 특히 답변생성 agent의 프롬프트 engineering을 통해서만  원하는 형식의 답변을 얻는 것이 어렵고,이에 따라 초기  LangGraph route 및 노드설계의 중요성을 느낄 수 있었습니다.  

🐻‍❄️최준호 : \
LLM프로젝트를 다루면서, RAG뿐만 아니라, multi-agent라는 것을 경험하게 되었습니다. 사용자가 입력한 키워드를 추출하여 이에 맞는 SQL문을 작성하고, 이를 llm_prompt 등에 적용할 수 있는 agent를 구축하였는데, 사용자가 입력할 수 있는 키워드 조합이 다양해서 어떤 기준으로 추출해야하는지 어려움이 있었습니다. chatgpt 등 llm에게 물어보고, 또 물어봐서 원하는 응답을 도출할 수 있었습니다. 이 과정을 통해 ai 개발자의 역할에 대해 알게되었습니다.  

🦊박세영 : \
드디어 끝났습니다! RDB 조회 agent 코드를 작성하면서 컬럼을 조금 더 상세히 나눴다면 필터링이 조금 더 편했을 거라는 아쉬운 기분이 들었습니다. 다음 데이터 전처리 시에는 전처리 이전에 해당 데이터의 목적을 분명히 나눠야겠습니다. 그럼에도 다른 agent와 노드의 실행으로 답변이 보완되는 것을 보고 우리 프로젝트의 설계의 완성도가 높다는 생각이 들었습니다.  
