...
class GraphNodes:
    ...
    def append_sources_to_answer(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Updated state with modified 'answer' including sources
        """
        ...
        for sentence in answer.split("\n"):
            if "출처" in sentence:
                ...
                for i in range(len(sources) -1, -1, -1):
                    target = target.replace(str(i + 1), f"📄 {sources[i]['bank_name']} - {sources[i]['product_name']} ")
                # 출저: => 뒤에 나오는 "출처" 키워드 중복 제거
                target = target.replace("출처 ", "")
                final_doc += target
            else:
                final_doc += sentence
                final_doc += "\n"
